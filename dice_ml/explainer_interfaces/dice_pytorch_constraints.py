...
class DicePyTorchConstraints(ExplainerBase):
    ...
    def _generate_counterfactuals(self, query_instance, total_CFs,
                                  ...
                                  diversity_weight=1.0, categorical_penalty=0.1, constraint_weight=1.0,
                                  algorithm="DiverseCF",
                                  features_to_vary="all",
                                  permitted_range=None, yloss_type="hinge_loss",
                                  diversity_loss_type="dpp_style:inverse_dist",
                                  feature_weights="inverse_mad", optimizer="pytorch:adam", learning_rate=0.05,
                                  min_iter=500,
        ...
        if [proximity_weight, diversity_weight, categorical_penalty, constraint_weight] != self.hyperparameters:
            self.update_hyperparameters(proximity_weight, diversity_weight, categorical_penalty, constraint_weight)
    ...
    def update_hyperparameters(self, proximity_weight, diversity_weight, categorical_penalty, constraint_weight):
        """Update hyperparameters of the loss function"""

        self.hyperparameters = [proximity_weight, diversity_weight, categorical_penalty, constraint_weight]
    ...
    def constraint_dist(self, x_hat):
        """Compute constraint distance """
        NUM_USERS = 10000
        ALPHA = 1000
        BETA = 7.5
        TAX_BAR = 4000
        ...
        def tax_income(income):
            income *= self.data_interface.data_df['income'].max()
            if income < TAX_BAR:
                return 0
            return 0.4 * ((income / (ALPHA + NUM_USERS * BETA)) ** 2)

        def cat_penalty(x):
            if (x[3] == 1) or (x[4] == 1) and (x[-2] == 1):
                return torch.tensor(1.0)
            if (x[5] == 1) or (x[6] == 1) and (x[-1] == 1):
                return torch.tensor(1.0)
            return torch.tensor(0.0)
        def tax_penalty(x):
            x1 = x.clone()
            x1[1] = int(tax_income(x1[0].item())*1000) / self.data_interface.data_df['tax'].max()
            return torch.sum(torch.mul((torch.abs(x - x1)), self.feature_weights_list), dim=0)
        # print(cat_penalty(x_hat))
        # return tax_penalty(x_hat) + 3*cat_penalty(x_hat)
        # return tax_penalty(x_hat)
        return 0
        # x1 = project(x_hat)
        # tax known only loss
        # x1[1] = int(tax_income(x1[0].item())*1000) / self.data_interface.data_df['tax'].max()
    ...
    def compute_constraints_loss(self):
        ...
        for i in range(self.total_CFs):
            constraints_loss += self.constraint_dist(self.cfs[i])
        # return constraints_loss / (torch.mul(len(self.minx[0]), self.total_CFs))
        return constraints_loss / self.total_CFs
    ...
    def compute_loss(self):
        ...
        self.constraints_loss = self.compute_constraints_loss() if self.constraint_weight > 0 else 0.0
        ...
        self.loss = self.yloss + (self.proximity_weight * self.proximity_loss) - \
                    ...
                    (self.constraint_weight * self.constraints_loss)
        return self.loss
    ...
    def find_counterfactuals(self, query_instance, desired_class, optimizer, learning_rate, min_iter,
        ...
        for loop_ix in range(loop_find_CFs):
            ...
            while self.stop_loop(iterations, loss_diff) is False:
                ...
                for ix in range(self.total_CFs):
                    for jx in range(len(self.minx[0])):
                        self.cfs[ix].data[jx] = torch.clamp(self.cfs[ix][jx], min=self.minx[0][jx],
                                                            max=self.maxx[0][jx])
